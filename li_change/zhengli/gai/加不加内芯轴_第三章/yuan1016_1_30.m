%% %beta画图，30度 tanhuang轴 -lr==0.0865  中段out_der=0.0299 out_huchang=0.3327
max=0;
f=[99,99,99];
pps=1;
%alpha_package=[60*pi/180;75*pi/180;90*pi/180];
alpha_package=[30*pi/180;32.5*pi/180;35*pi/180];
beta_stack=[0.1*pi/180;2.93*pi/180;10.85*pi/180];
%for alpha_change=1:size(alpha_package,1)
for alpha_change=1
alpha=alpha_package(alpha_change);
ei=1;f1=f(alpha_change);f1_ei=f1/ei;disp=1;
...
% beta_stack1=linspace(start_beta,end_beta,3);
...
%    plot(x_start,y_start,'.','Markersize',11,'Color',[0 0 0])
   %text(xp+0.01,yp+0.01,strcat('f',num2str(j))) ;
...
%    plot(final_disp(size(x_zhuan2,1)+1,1),final_disp(size(x_zhuan2,1)+1,2),'.','Markersize',11,'Color',[0 0 0])
    %text(final_disp(1,1),final_disp(1,2),strcat('F',num2str(j)),'FontSize',13);
