%% 同1015，用简单形式列写方程，得到相同力，弯曲角变大，弧长变大的推论
clear all;clc
figure; 
f=1;pps=1;
...
ia=[1:3:22];
inp=ia/180*pi;
%inp=[4/180*pi;7/180*pi;10/180*pi]';
%inp=10/180*pi;
% for j=1:size(inp,2)
%     beta_long(inp(j))-alpha/2
% end
...
out_sum=zeros(size(inp,2),3);
%% 每一个beta
for j=1:size(inp,2)
    %% 求a点
    %beta_transform=beta_long(inp(j))-alpha/2;
    beta_transform=inp(j);
[rao_displace(j,1),rao_displace(j,2),rao_displace(j,3)] =erjie_gai_func( f5,f6,beta_transform);
    xa=rao_displace(j,2);
    ya=rao_displace(j,3);
...
%     %% huchang
     out_sum(j,1)=rao_displace(j,1)*4+zhili_displace1(end,1);%相同

%     %% 中段形变
%     out_der=sqrt(xp^2+yp^2-(final_disp(1,1)^2+final_disp(1,2)^2)/4);
      out_sum(j,2)=sqrt(xp^2+yp^2-x_zhong^2-y_zhong^2);
%         %% 管道半径
out_sum(j,3)=x_zhong*2;
%     lr=final_disp(1,1);
    %% plot
...
plot(ia',out_sum(:,2))
legend({'弧长','夹角'},0)
plot(ia',out_sum(:,1),'.','Markersize',11,'Color',[0 0 0])
plot(ia',out_sum(:,2),'.','Markersize',11,'Color',[0 0 0])
