...
start_f=1.4;
end_f=2;
%f_stack=linspace(0.8,2,floor((end_f-start_f)/0.05)+1);
f_stack=linspace(start_f,end_f,floor((end_f-start_f)/0.001)+1);
%f_stack(175)---1.5740
ei=1;
%% 改变beta
...
%         %% 管道半径
        out_sum(i_a,i_f,i_b,3)=x_zhong*2;
...
set1=out_sum(i_a,floor(size(f_stack,2)/2),i_b,3);
for indexx=1:size(f_stack,2)
 if(abs(out_sum(i_a,indexx,i_b,3)-banjing)<abs(set1-banjing))
        %flag(i_f)=1;
        set1=out_sum(i_a,indexx,i_b,3);
        set1_index=indexx;
 end
end
