...
f51=f5*cos(alpha/2)+f6*sin(alpha/2)%%%0
f61=-f5*sin(alpha/2)+f6*cos(alpha/2)%%%%%-1.4142
[rao_displace(1,1),rao_displace(1,2),rao_displace(1,3)] =erjie_weifenfangcheng_func( f51,f61,beta_long,zui_length,disp )


fin_2=[0:0.05:beta_long]';
    point2=size(fin_2,1);
    force_disp2=zeros(point2,2);
    r2_displace=zeros(point2,3);
    force_disp2(:,1)=f5*cos(beta_long-fin_2)+f6*sin(beta_long-fin_2);%%f7
    force_disp2(:,2)=-f5*sin(beta_long-fin_2)+f6*cos(beta_long-fin_2);%%f8
    ...
    r2_displace(i,3)=quad(hy_theta,0,fin_2(i))
        ...
        theta_zhuan2=(pi/2-(beta_long)+fin_2);
    xa=rao_displace(1,2);
    ya=rao_displace(1,3);
    %%%
